...
export class ListaPokeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.pokeservice.getPokemones(this.offset).subscribe((data) => {
      this.pokemones = data;
      console.log(data);
    });
  }
  ...
  aumentarPokemon() {
    this.offset += 20;
    this.pokeservice.getPokemones(this.offset).subscribe((data) => {
      this.pokemones = this.pokemones.concat(data);
    });
  }
  agregarTipo(tipo: string) {
    ...
    this.pokeservice.getTipo(this.arregloTipos).subscribe((data) => {
      console.log(data);
      this.pokemones = data;
    });
  }
